...
const Carousel = (props) => {
  ...
  return (
    <div className="relative">
      {currentIndex != 0 && (
        <button
          className="absolute z-10 top-1/2 left-2 bg-white text-gray-900 rounded-full p-4 mt-96 transform -translate-y-1/2 hover:shadow-md focus:outline-none transition-all duration-500 ease-in-out"
          onClick={prevImage}
        >
      ...
      {currentIndex + 3 != props?.data?.length && (
        <button
